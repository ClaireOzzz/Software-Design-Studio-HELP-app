...
export const ML_REQUEST_HOST = () => {
  return axios.create({
    ...
    // baseURL: "https://create-request-ml-service-qdur742kca-as.a.run.app/"
  });
}
...
export const CHATBOT_HOST = () => {
  return axios.create({
    ...
    // baseURL: `https://gebirah-help-chatbot-qdur742kca-as.a.run.app`
  });
}

export const UXO_OBJECT_DETECTION_HOST = () => {
  return axios.create({
    baseURL: process.env.NODE_ENV === "production" ? `https://gebirah-help-uxo-detection-qdur742kca-as.a.run.app`: `http://localhost:9000`
...
export const getAllRequests = async (userid) => {
  return await API_HOST().get(`/api/v1/users/${userid}/feed/requests?user_id=`);
};

// get request details by id
export const getRequestDetails = async (id) => {
  return await API_HOST().get(`/api/v1/requests/${id}`);
};

// get feed requests with filters
// to refactor 
export const getFilteredRequest = async (userid, categories, search, proximity, lat, lng) => {
  let uri = `/api/v1/users/${userid}/feed/requests?user_id=${userid}`
...
export const createRequest = async (payload) => {
  return await API_HOST().post('/api/v1/requests', payload);
}

// delete existing request
export const deleteRequest = async (id, requestID) => {
  return await API_HOST().delete(`/api/v1/requests/${requestID}`);
}

// edit existing request
export const editRequest = async (id, requestID) => {
  return await API_HOST().put(`/api/v1/requests/${requestID}`);
}
...
export const createNotification = async (userID, requestID) => {
  return await API_HOST().post(`/api/v1/users/${userID}/feed/requests/${requestID}/accept`);
};
...
export const getLabelsAndSummariesML = async (text) => {
  return await ML_REQUEST_HOST().post('api/v1/ml_gen_title_and_categorization/predict', { "text": text });
}
...
export const getUXOBoundingBoxes = async(imageBytes) => {
  return await UXO_OBJECT_DETECTION_HOST().post('/api/v1/uxo_detections/predict', { "imageBytes": imageBytes });

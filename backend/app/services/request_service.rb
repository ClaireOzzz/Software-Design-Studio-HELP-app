class RequestService
    ...
    def accept_request(donor_id)
        if @request.update_request_status("accepted") && User.update_outreach_status(donor_id, @request.id, "pending") 
            return connect_users(donor_id)
        else
            puts "Something when wrong when accepting request!"
            nil
        end
    end
    ...
    def connect_users(donor_id)
        ...
        # if Connection already exists, abort

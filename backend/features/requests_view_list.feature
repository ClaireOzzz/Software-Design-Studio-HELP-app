...
Scenario: Accepting a request 
    ...
    When he clicks the Continue button 
    Then he should see "THANK YOU FOR REACHING OUT!"

@happy_path 
Scenario: Backing from request details 
    Given I log in as "OksanaKovalenko"
    Given Ludwig Gorny is on the feed page
    When Ludwig Gorny clicks on the first request
    And details for the first request is visible 
    When he clicks the close request button 
    Then he should be on the feed page
